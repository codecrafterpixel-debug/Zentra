  ...
  if (resource === "products") {
    ...
    if (id) {
      if (req.method === "PUT") {
        ...
        if ("originalPrice" in payload) updates.original_price = Number(payload.originalPrice);
        if ("stock" in payload) updates.stock = payload.stock;
        if ("trending" in payload) updates.trending = Boolean(payload.trending);
        if ("newArrival" in payload) updates.new_arrival = Boolean(payload.newArrival);
